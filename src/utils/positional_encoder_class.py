...
class MYPositionalEncoder4D(pl.LightningModule):
    ...
    def forward(
        self, shape_of_3dpositions: list, shape_of_1dpositions: list
    ) -> torch.Tensor:
        # B, SqL_l, Ch = masked_optimized_latent_codes_reshaped.shape
        ...
        del z3d
        z3d_positionally_encoded_re = rearrange(
            z3d_positionally_encoded, "B D H W Ch -> B (D H W) Ch"
        ).to(self.device)
        ...
        z1d = torch.zeros(
            (batch_size_, DHW_, Ch_), dtype=torch.float32, device=self.device
        )
        z1d_positionally_encoded = self.positional_encoder3D(z1d)
        del z1d
        z1d_positionally_encoded_re = rearrange(
            z1d_positionally_encoded, "B DHW Ch -> B (DHW) Ch"
        ).to(self.device)
        del z1d_positionally_encoded

        assert z3d_positionally_encoded_re.shape == z1d_positionally_encoded_re.shape
        # concat
        z4d_positionally_encoded = torch.cat(
            (z3d_positionally_encoded_re, z1d_positionally_encoded_re), dim=1
        )
