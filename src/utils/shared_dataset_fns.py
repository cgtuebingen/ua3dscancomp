...
def add_to_combined_data(cu3d_instance, data: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], partial_mesh: Tuple[torch.Tensor, torch.Tensor], resolution: int, device: str) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def add_to_combined_data_grid_search(cu3d_instance, data: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], partial_mesh: Tuple[torch.Tensor, torch.Tensor],
                                     resolution: int, device: str,
                                     weight_dist: float = 100.0, wight_dot_product: float = 50.0,
...
def normalize_combined_data(data: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], device: str) -> Tuple[torch.Tensor, torch.Tensor]:
    mu_combined, weighted_uncertainty_combined, w_combined = data

    normalized_uncertainty_combined = ((weighted_uncertainty_combined / w_combined)).to(device=device)
...
def calculate_sdf_and_dots(cu3d_instance, vertices: torch.Tensor, faces: torch.Tensor, device: str):
    if device == 'cpu':
        sdf, dots = calculate_sdf_and_dots_cuda(cu3d_instance, vertices.cpu().numpy(), faces.cpu().numpy(), device)
    else:
...
def do_sdf_on_marched_sdf(cu3d_instance, gt_sdf_voxel: np.ndarray, device: str) -> dict:
    ...
    marched_sdf, marched_dots = calculate_sdf_and_dots(cu3d_instance, marched_vertices_, marched_triangles_, device)
    # we do not care for the marched_dots
    return {"marched_sdf": marched_sdf, "marched_vertices": marched_vertices_, "marched_faces": marched_triangles_}
...
def pick_camera_views_per_object(num_views: int = 7, num_test_split: int = 5000, num_views_to_generate: int = 20):
    ...
    random_views = torch.randint(0, chunk_size, (num_test_split, num_views)) + torch.arange(0, chunk_size * num_views, chunk_size).unsqueeze(0)
    backup_views = torch.randint(chunk_size*(num_views-1), len(view_dirs), (num_test_split, num_views))
    random_views[random_views >= len(view_dirs)] = backup_views[random_views >= len(view_dirs)]
    ...
    # torch.save({'mvps': mvps, 'view_dirs': view_dirs, 'shuffled_views': shuffled_views}, "/graphics/scratch3/staff/zakeri/LMDBs/filtered_objaverse_joined_lmdb/test_views/views.pkl") # for objaverse
    # torch.save({'mvps': mvps, 'view_dirs': view_dirs, 'shuffled_views': shuffled_views}, "/graphics/scratch3/staff/zakeri/LMDBs/Shapenet_lmdb/test_views/views.pkl") # for shapenet
