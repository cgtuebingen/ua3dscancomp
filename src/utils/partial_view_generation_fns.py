...
def rasterize(glctx, mtx: torch.Tensor, pos: torch.Tensor, pos_idx: torch.Tensor, resolution: int):
...
def exclude_invalid_triangle_id(triangle_id: torch.Tensor, faces: torch.Tensor) -> torch.Tensor:
...
def calculate_surface_normals(vertices: torch.Tensor, faces: torch.Tensor) -> torch.Tensor:
    v0 = vertices[:, faces[:, 0]]
    v1 = vertices[:, faces[:, 1]]
    v2 = vertices[:, faces[:, 2]]
...
def generate_mvp_matrices(num_views: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    golden_ratio = (np.sqrt(5.0 + 1)) * 0.5
    phi = np.linspace(0, 2.0 * np.pi * num_views / golden_ratio, num_views, endpoint=False)
    ...
    mv = torch.cat([mv, homo.unsqueeze(0).unsqueeze(0).expand([num_views, 1, 4])], dim=1)
...
def compute_partial_views(vertices: torch.Tensor, face_indices: torch.Tensor, glctx: Union[dr.RasterizeGLContext, dr.RasterizeCudaContext], mvps: torch.Tensor, dirs: torch.Tensor, resolution: int = 64, device: str = 'cuda:0'):
    ...
    dot_product_condition = clean_faces_based_on_depth(dirs.unsqueeze(1).to(device=device), vertices, faces_rows)
