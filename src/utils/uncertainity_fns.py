...
def calculate_uncertainty_h8(dot_product_value, dist):
    unc_val = (dist * dist * 100.0) * (
        2 - torch.pow(dot_product_value, 21)
    ) + 50.0 * torch.square(1.0 - torch.abs(dot_product_value))

    return unc_val


def calculate_uncertainty_h8_grid_search(
    dot_product_value, dist, weight_dist: float = 100.0, wight_dot_product: float = 50.0
):
    unc_val = (dist * dist * weight_dist) * (
        2 - torch.pow(dot_product_value, 21)
    ) + wight_dot_product * torch.square(1.0 - torch.abs(dot_product_value))
...
def combine_distribution(
    ...
    w_combined: torch.float32,
    mu_combined: torch.float32,
    uncertainty_combined: torch.float32,
):
    sigma_i = uncertainty
    mu_i = sdf
...
def combine_distributions_with_incremental_mean(
    sdf_all: list[torch.Tensor], uncertainty_all: list[torch.Tensor]
...
def generate_coordinates(resolution: int = 128, value_range: int = 1):
    ...
    voxels = np.stack((x, y, z), axis=3) + (value_range - -value_range) / (
        2 * resolution
    )
...
def write_uncertainty_as_csv(
    file_name: str, gt_sdf_voxel: torch.Tensor, unc_values: torch.Tensor
):
    ...
    for p in range(points.shape[0]):
        ...
        string += (
            str(point_current[0].item())
            + ", "
            + str(point_current[1].item())
            + ", "
            + str(point_current[2].item())
            + ", "
            + str(gt_sdf_voxel[p].item())
            + ", "
            + str(unc_values[p].item())
            + "\n"
        )
