...
def decode_data_for_vis(
    data_vis: torch.Tensor, dict_arguments_of_variables: dict, fdecoder
):
    ...
    with torch.no_grad():
        decoded_data_vis = fdecoder(data_vis).to(
            device=fdecoder.device
        )  # [1, 64, 512, 2, 2, 2] -> [64, 1, 32, 32, 32]
        decoded_data_vis_reshaped = decoded_data_vis.reshape(
            [
                batch_size,
...
def generate_any_data_for_plotting(
    ...
    for i in range(len(keys)):
        ...
        decoded_data_vis_current_reshaped = decode_data_for_vis(
            data_vis_current, dict_arguments_of_variables, fdecoder
        )
        collected_decoded_data_vis_current_reshaped = (
            collect_sub_voxels_to_voxel_with_batch(
                decoded_data_vis_current_reshaped, resolution
            )
        )
        assert collected_decoded_data_vis_current_reshaped.shape == (
            batch_size,
            resolution,
            resolution,
            resolution,
...
def collect_any_generated_data_for_plotting(
    ...
    for i in range(len(keys)):
        ...
        collected_data_current_array = (
            collected_data_current[batch_idx]
            .squeeze()
            .detach()
            .to(torch.float32)
            .cpu()
...
def march_any_results_every_n_epoch(
    dict_of_items: dict,
    selected_index: int,
    current_epoch: int,
    global_step: int,
