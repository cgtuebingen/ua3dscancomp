...
class EvalObjaverse:
    def __init__(
        ...
        pretrained_model = cp.load_from_checkpoint(
            checkpoint_path=ckpt_path, map_location="cpu"
        )
    ...
    def forward(
        ...
        concatenated_latent_codes_reshaped = concatenated_latent_codes.reshape(
            [batch_size, self.number_of_sub_voxels, 2 * 8 * self.latent_dim]
        )
    ...
    def encode_stuff(
        ...
        uncertainty_latent_codes_r = uncertainty_latent_codes.reshape(
            [batch_size, self.number_of_sub_voxels, self.latent_dim, 2, 2, 2]
        )
    ...
    def fwd(self, batch: list, test: bool) -> tuple[Tensor, Tensor, Tensor]:
        (
            ...
            folder_name,
            combined_sdf_voxel,
            combined_uncertainty_voxel_normalized,
        ) = batch
        batch_size = object_indices.shape[0]
        # -----------------------------------------------------------------------------------------------------------------------------------------
        combined_sdf_sub_voxels = pp_fns.sub_divide_gt_and_normalize(
            combined_sdf_voxel, self.number_of_sub_voxels, self.target_resolution
        )
        ...
        sdf_latent_codes, uncertainty_latent_codes = self.encode_stuff(
            combined_sdf_sub_voxels, combined_uncertainty_voxel_normalized, False
        )
    ...
    def export_gt(self, data_to_export):
        ...
        gt_mesh_file_name = os.path.join(
            gt_dir,
            "GT"
            + "_"
            + folder_name
    ...
    def test(self) -> None:
        ...
        for i in tqdm(
            range(self.min_range, self.max_range, 1), "Objaverse Test Samples:"
        ):
            ...
            combined_uncertainty_voxel_normalized = (
                combined_uncertainty_voxel_normalized.unsqueeze(0).to(
                    device=self.device
                )
            )
            gt_sdf_latent_codes = (
                torch.from_numpy(gt_sdf_latent_codes)
                .unsqueeze(0)
            ...
            if False:
                ...
                data_to_export: dict = {
                    ...
                    "object_index": object_indices.detach().cpu().item(),
                    "mesh_name": mesh_name,
            ...
            for k in range(len(keys)):
                ...
                if current_key.startswith("Transformer") or current_key.startswith(
                    ...
                    make_mcubes_from_voxels_ply_with_pad(
                        collected_data_current, export_file_name
                    )
                if current_key.startswith("Transformer") and export_file_name.endswith(
                    ".ply"
                ):
            ...
            un_normalized_uncertainty_voxel = (
                combined_uncertainty_voxel_normalized.squeeze(0) * 50
            ) + 50
            # This is only for visualization purpose
            clipped_combined_sdf_voxel = (
                combined_sdf_voxel.squeeze(0)
                - torch.clip(un_normalized_uncertainty_voxel - 8, min=0.0) * 0.003
            )
            input_dir = os.path.join(
                self.common_obj_dir,
                "Inputs",
                "Input" + "_num_views-" + str(self.num_views_for_test),
            )
            if not os.path.isdir(input_dir):
                os.mkdir(input_dir)
            input_export_file_name = os.path.join(
                input_dir,
                "Input"
                + "_"
                + folder_name
                + "_"
                + mesh_name.rsplit("_")[0]
                + "_ObjID="
                + str(selected_index)
                + ".ply",
            )
            if not os.path.isfile(input_export_file_name):
                make_mcubes_from_voxels_ply_with_pad(
                    clipped_combined_sdf_voxel.cpu().numpy(), input_export_file_name
                )
            ...
            gt_dir = os.path.join(self.common_obj_dir, "GT", "obj_dir")
            gt_export_file_name = os.path.join(
                gt_dir,
                "GT"
                + "_"
                + folder_name
            ...
            dict_args_eval["Completed_voxel"] = collected_data_dict_for_plotting[
                "Transformer"
            ]
    ...
    def setup(self):
        ...
        self.test_dataset = TESTLMDBOBJAVERSEPARTIALVIEWS(
            self.mesh_path,
            self.views_dict_path,
            self.test_lmdb_path,
            self.obj_dir,
            self.image_resolution,
            self.num_views_for_test,
            self.resolution,
            device=self.device,
        )
