...
class SDFtoSDF(pl.LightningModule):
    ...
    def training_step(self, batch: list, batch_idx: int) -> dict:
        object_indicis, mesh_file_name, gt_sdf_voxels_transformed, distance_per_voxels, left, x_offset, top, y_offset, front, z_offset_copy, folder, label, dataset_index, sub_folder_index = batch
    ...
    def validation_step(self, batch: list, batch_idx: int) -> None:
        ...
        for b in range(0, self.hparams.batch_size, 2):
            ...
            if selected_index in my_selected_meshes:
                ...
                gt_sdf_voxel_transformed_image_list, gt_sdf_voxel_transformed_title_list = plot_v(gt_sdf_voxel_, number_of_slices, self.hparams.target_resolution, "gt_sdf_voxel_transformed", plot_range=[-1, 1])
                # ----
                ...
                predicted_image_list, predicted_title_list = plot_v(predicted_, number_of_slices, self.hparams.target_resolution, "sdf_voxel_reconstructed", plot_range=[-1, 1])

                # ----
                diff = abs(np.subtract(predicted_, gt_sdf_voxel_))
                diff_ = np.asarray(diff).astype(float)
                diff_image_list, diff_title_list = plot_v(diff_, number_of_slices, self.hparams.target_resolution, "diff", plot_range=[-1, 1])
                # ----
                del predicted_
                del gt_sdf_voxel_
                ...
                for sl in range(0, number_of_slices, 1):
                    ...
                    plot = torch.cat([image_re.to(device=self.device), image_gt.to(device=self.device), image_diff.to(device=self.device)], -1)
                    del image_re
                    del image_gt
                    del image_diff
                    # show in tensorboard
    ...
    def configure_optimizers(self):
        ...
        print("\nmax_steps", max_steps)
        scheduler = {"scheduler": CosineAnnealingLR(optimizer, T_max=max_steps, eta_min=0, last_epoch=-1)}
