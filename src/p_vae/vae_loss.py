...
class VAELoss(torch.nn.Module):
    ...
    def forward(self,
                 gt_sequence: TensorType["Batch", "BlockDuration", "BlockHeight", "BlockWidth", "RGB"],
                 pred_sequence: TensorType["Batch", "BlockDuration", "BlockHeight", "BlockWidth", "RGB"],
                 posterior: p_vae.distribution.Distribution, global_step: int, log_prefix: str = "training",
        ...
        negative_log_likelihood_loss = (reconstruction_loss / torch.exp(self.logvar) + self.logvar)  # L1 loss as main loss, converts it into a negative log likelihood
        kullback_leibler_loss = posterior.KL()
        kullback_leibler_loss = (torch.sum(kullback_leibler_loss) / kullback_leibler_loss.shape[0])
        ...
        loss = (negative_log_likelihood_loss + kullback_leibler_loss * self.kulbak_leibler_weight)
...
class LossPerVoxel(torch.nn.Module):
    ...
    def forward(self,
                 ...
                 pred_sequence: TensorType["Batch", "BlockDuration", "BlockHeight", "BlockWidth"],
                 ) ->TensorType["Batch", 1]:

        reconstruction_loss_per_batch = torch.empty([gt_sequence.shape[0], 1])
        for b in range(gt_sequence.shape[0]):
            reconstruction_loss_per_batch[b] = torch.abs(gt_sequence[b] - pred_sequence[b]).mean()  # L1
