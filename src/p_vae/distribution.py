...
class Distribution:
    ...
    def __init__(self, parameters: TensorType["Batch", "TokenSize"]):
        ...
        self.logvar = torch.clamp(self.logvar, -30, 20)   # torch.clamp(self.logvar, -30, 20) was used for sdf
    ...
    def sample(self) -> TensorType["Batch", "DistSize"]:
       ...
       """
        rand_ = torch.randn(self.mean.shape, dtype=self.parameters.dtype, device=self.parameters.device)
        return self.mean + self.std * torch.randn(self.mean.shape, dtype=self.parameters.dtype, device=self.parameters.device)
    ...
    def KL(self, other=None) -> TensorType["Batch"]:
       ...
       """
        if other is None:
            return 0.5 * torch.sum(self.mean * self.mean + self.var - 1 - self.logvar, dim=[1])
        else:
            return 0.5 * torch.sum((self.mean - other.mean) * (self.mean - other.mean) / other.var + self.var / other.var - 1 - self.logvar + other.logvar, dim=[1], )
