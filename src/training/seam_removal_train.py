...
from utils import loss_helper_fns as l_fn, transformer_visualizations as tv
...
from utils.subvolume_devision import collect_sub_voxels_to_voxel_with_batch
...
class RemoveSeams(pl.LightningModule):
    def __init__(
        ...
        if pre_trained:
            pretrained_model = sps.load_from_checkpoint(
                checkpoint_path=first_transformer_ckpt, map_location="cpu"
            )
    ...
    def call_seam_removal_with_conv3d(
        self, transformer_output_sequence_up: torch.Tensor
        ...
        with torch.no_grad():
            decoded_trans_output = self.fdecoder(
                transformer_output_sequence_up_reshaped
            )
        decoded_trans_output_reshaped = decoded_trans_output.reshape(
            [
                batch_size,
                self.number_of_sub_voxels,
        ...
        decoded_trans_output_collected = collect_sub_voxels_to_voxel_with_batch(
            decoded_trans_output_reshaped, self.hparams.resolution
        )  # [B, 128, 128, 128]
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            assert (
                ...
                == concatenated_latent_codes_reshaped.shape
            )
        ...
        del (
            concatenated_latent_codes,
            concatenated_latent_codes_reshaped,
            z_positionally_encoded_re,
            transformer_input_sequence,
        )

        conv3d_output, decoded_trans_output_collected = (
            self.call_seam_removal_with_conv3d(transformer_output_sequence)
        )
    ...
    def fwd(
    ...
    ) -> tuple[dict[str, Any], tuple[Tensor, Tensor, Tensor, Tensor]]:
        with torch.no_grad():
            (
                object_indices,
                mesh_file_name,
        ...
        conv3d_output, decoded_trans_output_collected = self.forward(
        ...
        consistency_loss = torch.mean(
            torch.abs(conv3d_output - decoded_trans_output_collected)
        )
        ...
        return (
            ...
            (
                ...
                decoded_trans_output_collected,
                conv3d_output,
            ),
        )
    ...
    def plot_march_and_login_tensorboard(
        ...
        data_dict_for_vis["diff_decoded_trans_output_vs_conv3d_output"] = (
            diff_decoded_trans_output_vs_conv3d_output
        )

        data_dict_for_vis["conv3d_output"] = dict_arguments_of_variables[
            "conv3d_output"
        ]
        data_dict_for_vis["decoded_trans_output_collected"] = (
            dict_arguments_of_variables["decoded_trans_output_collected"]
        )
