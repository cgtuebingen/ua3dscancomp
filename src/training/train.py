...
class CompletePartialScans(pl.LightningModule):
    def __init__(
        ...
        self.mapping_down = nn.Linear(self.penc_channels + 2 * 8 * self.hparams.latent_dim, self.hparams.dim_size)
    ...
    def fwd(self, batch: list, train: bool, val: bool, test: bool) -> tuple[dict[str, Any], tuple[Tensor, Tensor, Tensor]]:
        ...
        gt_sdf_latent_codes_reshaped = gt_sdf_latent_codes.reshape([batch_size, self.number_of_sub_voxels, 8 * self.hparams.latent_dim])
        assert transformer_output_sequence_up_reshaped.shape == gt_sdf_latent_codes_reshaped.shape

        # calculate losses:
        loss_l1 = self.l1_loss(transformer_output_sequence_up_reshaped, gt_sdf_latent_codes_reshaped)
        # log losses
        if train:
            loss_l1 = self.l1_loss(transformer_output_sequence_up_reshaped, gt_sdf_latent_codes_reshaped)
        ...
        return (loss_dict, (sdf_latent_codes, uncertainty_latent_codes, transformer_output_sequence_up))
    ...
    def plot_march_and_login_tensorboard(self, dict_arguments_for_vis: dict, dict_arguments_of_variables: dict, object_indices: torch.Tensor, batch_size: int) -> None:
        ...
        data_dict_for_vis["diff_transformerOutput_sdfLatentCodes"] = diff_transformer_output_vs_latent_codes
